properties {
  $Build_Solution = 'SqlToGraphitePlugin-Oracle.sln'
  $Debug = 'Debug'
  $pwd = pwd
  ...
  $version = '0.1.0.1'
  $nuspecFile = "SqlToGraphitePlugin-Oracle.nuspec"
}

task default -depends Init, StartOracle, Clean, Compile, Test, Package , Report
...
task Package {
   if ((Test-path -path $Build_Artifacts -pathtype container) -eq $false)
	...
	Copy-item .\src\SqlToGraphite.Plugin.Oracle\output\SqlToGraphite.Plugin.Oracle.dll $Build_Artifacts\
	Copy-item SqlToGraphitePlugin-Oracle.nuspec $Build_Artifacts\
